...
const getBySport = async (sportId) => {
	return await GameModal.findAll({
		where: { sportId },
		include: [
			{ model: TeamModal, as: "mainTeam", foreignKey: "mainTeamId", include: { model: UserModel, as: "user", foreignKey: "userId", attributes: ["id", "image"] } },
			{ model: TeamModal, as: "opponentTeam", foreignKey: "opponentTeamId", include: { model: UserModel, as: "user", foreignKey: "userId", attributes: ["id", "image"] } },
